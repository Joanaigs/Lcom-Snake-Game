...
int issueCommand(uint8_t cmd, int port) {
...
int (mouse_enable_data_report)() {
  for (int i = 0; i < 5; ++i) {
    if (issueCommand(WRITE_BYTE_TO_MOUSE, KBC_CMD) != 0) {
      return 1;
    }
    if (issueCommand(ENABLE_DATA_REPORTING, OUTPUT_BUF) != 0) {
      return 1;
    }
    if(util_sys_inb(OUTPUT_BUF, &outuputByte) != 0){
      return 1;
    }
    if (outuputByte == ACK) {
      return 0;
...
int (mouse_disable_data_report)() {
  for (int i = 0; i < 5; ++i) {
    ...
    if (issueCommand(DISABLE_DATA_REPORTING, OUTPUT_BUF) != 0) {
      return 1;
    }
    if(util_sys_inb(OUTPUT_BUF, &outuputByte) != 0){
      return 1;
